...
int main(void) {
    ...
    printf("sensor %d: program started\n", SENSOR_INDEX);
    ...
    for (int attempt = 1; attempt <= 10; attempt++) {
        printf("sensor %d: IIC check attempt %d\n", SENSOR_INDEX, attempt);
    ...
    if (!iic_ready) {
        printf("sensor %d: IIC check failed\n", SENSOR_INDEX);
        iic_destroy(IIC0);
        pynq_destroy();
        return EXIT_FAILURE;
    }

    printf("sensor %d: IIC bus stable\n", SENSOR_INDEX);

    // Now initialize VL53L0X ranging.
    bool initialized = false;

    for (int attempt = 1; attempt <= 5; attempt++) {
        printf("sensor %d: init attempt %d\n", SENSOR_INDEX, attempt);
    ...
    while (1) {
        ...
        if (read_distance_mm(&distance) && distance > 0 && distance <= OUT_OF_RANGE_MM) {
            printf("sensor %d: %u mm\n", SENSOR_INDEX, distance);
        } else {
            printf("sensor %d: out of range\n", SENSOR_INDEX);
        }
