...
static bool distance_sensor_ready = false;

int init_distance_sensor(void) {
    ...
    if (!iic_ready) {
        printf("distance sensor: IIC check failed\n");
        return 0;
    }
    ...
    if (!initialized) {
        printf("distance sensor: initialization failed\n");
        return 0;
    }

    distance_sensor_ready = true;
    return 1;
}
...
int get_distance(void) {
    ...
    if (!distance_sensor_ready) {
        return -1;
    }

    if (read_distance_mm(&distance) && distance > 0 && distance <= OUT_OF_RANGE_MM) {
        return (int)distance;
    }
...
void destroy_distance_sensor(void) {
    iic_destroy(IIC0);
    distance_sensor_ready = false;
}
